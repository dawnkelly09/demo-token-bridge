...
const MOONBEAM_PRIVATE_KEY = process.env.MOONBEAM_PRIVATE_KEY;
const SEPOLIA_PRIVATE_KEY = process.env.SEPOLIA_PRIVATE_KEY;
const MOONBEAM_RPC_URL = process.env.MOONBEAM_RPC_URL! || 'https://rpc.api.moonbase.moonbeam.network';
...
export function getMoonbeamWallet(): ethers.Wallet {
  return new ethers.Wallet(MOONBEAM_PRIVATE_KEY!, new ethers.JsonRpcProvider(MOONBEAM_RPC_URL));
}
export function getSepoliaWallet(): ethers.Wallet {
  return new ethers.Wallet(SEPOLIA_PRIVATE_KEY!, new ethers.JsonRpcProvider(SEPOLIA_RPC_URL));
}
